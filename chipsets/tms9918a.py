...
class TMS9918A:
    ...
    def _render_graphics1(self, out: bytearray) -> None:
        ...
        color_base = self._color_table_base()
        pattern_base = self._pattern_table_base()
    ...
    def _find_sprites_on_line(self, y: int) -> list[tuple[int, int, int, int, bool]]:
        sat_base = self._sprite_attribute_base()
        large_sprites = bool(self.registers[1] & 0x02)
    ...
    def _resolve_sprite_color(self, sprites: list[tuple[int, int, int, int, bool]], y: int, x: int) -> int:
        pattern_base = self._sprite_pattern_base()
        large_sprites = bool(self.registers[1] & 0x02)
        magnify = bool(self.registers[1] & 0x01)
        sprite_size = 8 << (int(large_sprites) + int(magnify))
        found_color: int | None = None
