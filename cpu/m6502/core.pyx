...
cdef class M6502Core:
    ...
    cdef inline void _set_flag(self, int mask, bint enabled):
        if enabled:
            self.P |= mask
        else:
            self.P &= ~mask

    cdef inline int _read_zp(self):
        return self.bus.read8(self._fetch8())

    cdef inline int _read_zp_x(self):
        return self.bus.read8((self._fetch8() + self.X) & 0xFF)

    cdef inline int _read_zp_y(self):
        return self.bus.read8((self._fetch8() + self.Y) & 0xFF)

    cdef inline int _read_abs(self):
        return self.bus.read8(self._fetch16())

    cdef inline int _read_abs_x(self):
    ...
    cdef inline int _read_ind_y(self):
    ...
    cdef inline void _write_zp_x(self, int value):
        self.bus.write8((self._fetch8() + self.X) & 0xFF, value)

    cdef inline void _write_zp_y(self, int value):
        self.bus.write8((self._fetch8() + self.Y) & 0xFF, value)

    cdef inline void _write_abs(self, int value):
        self.bus.write8(self._fetch16(), value)

    cdef inline void _write_abs_x(self, int value):
        self.bus.write8((self._fetch16() + self.X) & 0xFFFF, value)

    cdef inline void _write_abs_y(self, int value):
        self.bus.write8((self._fetch16() + self.Y) & 0xFFFF, value)

    cdef inline void _write_ind_x(self, int value):
        cdef int zp_addr = (self._fetch8() + self.X) & 0xFF
        cdef int addr = self.bus.read8(zp_addr) | (self.bus.read8((zp_addr + 1) & 0xFF) << 8)
        self.bus.write8(addr, value)

    cdef inline void _write_ind_y(self, int value):
        cdef int zp_addr = self._fetch8()
        cdef int base = self.bus.read8(zp_addr) | (self.bus.read8((zp_addr + 1) & 0xFF) << 8)
        self.bus.write8((base + self.Y) & 0xFFFF, value)

    cdef inline void _push8(self, int value):
        self.bus.write8(0x0100 | self.SP, value & 0xFF)
        self.SP = (self.SP - 1) & 0xFF

    cdef inline int _pop8(self):
        self.SP = (self.SP + 1) & 0xFF
        return self.bus.read8(0x0100 | self.SP)

    cdef inline void _push16(self, int value):
        self._push8((value >> 8) & 0xFF)
        self._push8(value & 0xFF)

    cdef inline int _pop16(self):
        cdef int lo = self._pop8()
        cdef int hi = self._pop8()
        return lo | (hi << 8)

    cdef void _adc(self, int value):
        value &= 0xFF
        cdef int carry_in = 1 if (self.P & FLAG_C) else 0
        cdef int total = self.A + value + carry_in
        cdef int result = total & 0xFF
        cdef bint overflow = (~(self.A ^ value) & (self.A ^ result) & 0x80) != 0
        cdef int lo, carry_lo, hi
        if self.P & FLAG_D:
    ...
    cdef void _sbc(self, int value):
        value &= 0xFF
        cdef int carry_in = 1 if (self.P & FLAG_C) else 0
        cdef int total = self.A - value - (1 - carry_in)
        cdef int result = total & 0xFF
        cdef bint overflow = ((self.A ^ result) & (self.A ^ value) & 0x80) != 0
        cdef int borrow, lo, borrow_hi, hi
        if self.P & FLAG_D:
            borrow = 1 - carry_in
    ...
    cdef inline void _compare(self, int reg, int value):
        cdef int result = (reg - (value & 0xFF)) & 0x1FF
        self._set_flag(FLAG_C, reg >= (value & 0xFF))
        self._set_zn(result & 0xFF)

    cdef inline int _branch(self, bint condition):
        cdef int offset = self._fetch8()
        cdef int old_pc
        if not condition:
            return 2
        old_pc = self.PC
    ...
    cdef inline int _asl_value(self, int value):
        self._set_flag(FLAG_C, (value & 0x80) != 0)
        value = (value << 1) & 0xFF
        self._set_zn(value)
        return value

    cdef inline int _lsr_value(self, int value):
        self._set_flag(FLAG_C, (value & 0x01) != 0)
        value = (value >> 1) & 0xFF
        self._set_zn(value)
        return value

    cdef inline int _rol_value(self, int value):
        cdef int carry_in = 1 if (self.P & FLAG_C) else 0
        self._set_flag(FLAG_C, (value & 0x80) != 0)
    ...
    cdef inline int _ror_value(self, int value):
        cdef int carry_in = 0x80 if (self.P & FLAG_C) else 0x00
        self._set_flag(FLAG_C, (value & 0x01) != 0)
        value = ((value >> 1) | carry_in) & 0xFF
        self._set_zn(value)
        return value

    cdef inline int _inc_value(self, int value):
        value = (value + 1) & 0xFF
        self._set_zn(value)
        return value

    cdef inline int _dec_value(self, int value):
        value = (value - 1) & 0xFF
        self._set_zn(value)
        return value

    cdef int _service_interrupt(self, int vector, int pushed_status):
